...
# to import the old names which would raise an ImportError (seen when
# running the simulation).  rather than juggling multiple branches we
# simply pull in whatever is available and alias it as ``Generator``
...
class FireSim:
    ...
    def __init__(self, input, n, location_sampler=random.choice,
                 ...
                 # fire_mover expects to receive a list of (loc,attrs) tuples
                 # and return a single pick.  ``random.sample`` requires a second
                 # ``k`` argument which we weren't providing; use ``choice``
                 # instead to pick a single item.
                 fire_mover=lambda a: random.choice(a),
    ...
    def update_person(self, person_ix):
        ...
        # Get occupied locations (excluding current person's location)
        occupied = {person.loc for person in self.people if person.alive and not person.safe and person.id != person_ix}

        target = p.move(nbrs, occupied=occupied)
        if not target:
            p.alive = False
            self.numdead += 1
            if self.verbose:
    ...
    def simulate(self, maxtime=None, spread_fire=False, gui=False):
        ...
        # schedule first update using the person's rate
        for i in range(len(self.people)):
            self.sim.sched(self.update_person, i, offset=0)
    ...
    def stats(self):
        ...
        '''
        print('\n' + '='*60)
        print(' ' * 20 + 'SIMULATION STATISTICS')
        print('='*60)

        def print_stat(label, value, unit=''):
            print(f" {label.ljust(35)} : {str(value).rjust(10)} {unit}")

        print_stat('Total People', self.numpeople)
        print_stat('People Safe', self.numsafe, '(✓)')
        print_stat('People Dead', self.numpeople - self.numsafe - self.nummoving, '(✗)')
        print_stat('People Injured/Moving', self.nummoving, '(!)')
        print('-' * 60)
        
        if self.avg_exit:
            print_stat('Average Evacuation Time', f"{self.avg_exit:.2f}", 's')
        else:
            print_stat('Average Evacuation Time', 'N/A')
            
        survival_rate = (self.numsafe / self.numpeople) * 100
        print_stat('Survival Rate', f"{survival_rate:.1f}", '%')
